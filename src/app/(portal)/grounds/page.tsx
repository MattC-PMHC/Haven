...
// Grounds Crew Portal — Work Order Dashboard
//
// Mobile-first design with large tap targets and clear status indicators.
// Shows assigned and in-progress work orders for the crew.
...
export default function GroundsCrewDashboard() {
                ...
                <div className="px-4 pb-4 space-y-3 border-t border-border pt-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-muted-foreground">{getCemeteryName(wo.cemetery_id)}</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <Clock className="size-3.5 text-muted-foreground" />
                      <span className="text-muted-foreground">~{wo.estimated_hours}h</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <ClipboardList className="size-3.5 text-muted-foreground" />
                      <span className="text-muted-foreground">{typeLabels[wo.work_order_type] ?? wo.work_order_type}</span>
                    </div>
                    <div>
                  ...
                  <div className="flex gap-2 pt-1">
                    {wo.status === "assigned" && (
                      <Button className="flex-1 rounded-xl h-12 text-base" disabled>
                        <Play className="size-5" />
                        Start Work
                      </Button>
                    )}
                    {wo.status === "in_progress" && (
                      <Button className="flex-1 rounded-xl h-12 text-base bg-success hover:bg-success/90" disabled>

// Middleware — role-based routing and auth enforcement.
//
...
function isAdminPath(pathname: string): boolean {
  // Admin paths are: /dashboard, /plots, /registry, /bookings, /maps,
  // /settings, /reports, /audit, /contacts, /documents, /work-orders,
  // /memorials, /rights, /notifications
  const adminPrefixes = [
    "/dashboard",
    "/plots",
    "/registry",
...
export async function middleware(request: NextRequest) {
  ...
  if (pathname === "/") {
    ...
    // Read role from app_metadata (set during signin/registration)
  ...
  if (!isAdmin && isPortalPath(pathname)) {
    const home = getHomeForRole(role)
    // If they're on a portal path that doesn't match their role's portal
